...
load_dotenv()
from db import init_db, get_summary, get_opportunities, log_event
from engine import run_mission_once, default_mission
from config import AUTO_SPEND_USD
...
def token():
    return USER + "." + hmac.new(SECRET.encode(), USER.encode(), hashlib.sha256).hexdigest()

def valid(t):
    if not t or "." not in t or not SECRET:
        return False
    u, s = t.rsplit(".", 1)
    expected = hmac.new(SECRET.encode(), u.encode(), hashlib.sha256).hexdigest()
    return u == USER and hmac.compare_digest(s, expected)
...
async def scheduler():
    while True:
        ...
        try:
            await run_mission_once(default_mission(), "scheduler")
        except Exception as e:
            log_event("scheduler_error", str(e))
...
async def index(req: Request):
    ...
    return HTMLResponse((ROOT / "index.html").read_text(encoding="utf8"))

@app.get("/login", response_class=HTMLResponse)
async def login_page():
    return HTMLResponse((ROOT / "login.html").read_text(encoding="utf8"))

@app.post("/login")
async def login(username: str = Form(...), password: str = Form(...)):
    if not PASSWORD or not SECRET:
        return HTMLResponse("JD_ADMIN_PASSWORD y JD_SESSION_SECRET deben estar configurados en Railway.", 500)
    ...
    r.set_cookie("jd_session", token(), httponly=True, secure=False, samesite="lax", max_age=604800)
    return r

@app.post("/logout")
async def logout():
    r = RedirectResponse("/login", 303)
    r.delete_cookie("jd_session")
    return r
...
async def mission(req: Request, payload: Mission):
    auth(req)
    if AUTO_SPEND_USD > 0:
        raise HTTPException(403, "Auto spending is disabled.")
    try:
